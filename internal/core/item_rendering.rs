...
pub struct PartialRenderer<'a, T> {
    ...
    /// Should actually be a region and not just a rectangle
    dirty_region: euclid::default::Box2D<f32>,
    /// This contains all the rectangle for debug purposes
    // FIXME should be put in a #[cfg]
    pub dirty_region_debug: Vec<Rect>,
    actual_renderer: T,
}
...
impl<'a, T> PartialRenderer<'a, T> {
    ...
    pub fn new(cache: &'a mut PartialRenderingCache, actual_renderer: T) -> Self {
        Self {
            ...
            dirty_region_debug: Default::default(),
            actual_renderer,
        }
    }
    ...
    fn mark_dirty_rect(&mut self, rect: Rect, offset: euclid::default::Vector2D<f32>) {
        if !rect.is_empty() {
            self.dirty_region_debug.push(rect.translate(offset));
...
/*
macro_rules! get_geometry_untracked {
    ($ty:ty, $obj:expr) => {{
        type Ty = $ty;
